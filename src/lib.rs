    ...
    D: Deserializer<'de>,
{
    ...
    // let ts = NaiveDateTime::parse_from_str(&s, "%s").map_err(serde::de::Error::custom)?;
    // chrono::offset::Utc.from_utc_datetime(&ts).unwrap()
}
...
mod tests {
    ...
    "_SOURCE_REALTIME_TIMESTAMP" : "1491389822657954"
        ...
        let p: JournalLog = serde_json::from_str(testdata_log)?;
        println!("{:?}", p);
        Ok(())
    }
